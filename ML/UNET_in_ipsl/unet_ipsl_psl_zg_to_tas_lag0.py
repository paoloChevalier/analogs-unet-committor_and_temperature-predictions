...
class CustomDataset(tf.keras.utils.Sequence):
    def __init__(self, psl_file, zg_file, tas_file, batch_size=32,lag=0,std_ref_psl=None,std_ref_zg=None):
    ...
    def __getitem__(self, idx):
        ...
        in_tensor = np.stack([zg,psl], axis=-1)
...
dataset_train = CustomDataset(psl_file_train, zg_file_train, tas_file_train, batch_size=64,lag=0)
dataset_val = CustomDataset(psl_file_val, zg_file_val, tas_file_val, batch_size=64,lag=0,std_ref_psl=dataset_train.psl_std, std_ref_zg=dataset_train.zg_std)
...
callbacks = [
    ...
    keras.callbacks.ModelCheckpoint("models/psl_zg_to_tas_ipsl_lag0.keras", save_best_only=True),
]
...
ds.to_netcdf("models/psl_zg_to_tas_ipsl_lag0.nc")
model.save("models/psl_zg_to_tas_ipsl_lag0.keras")
np.save('models/psl_zg_to_tas_ipsl_lag0.npy',history.history)
