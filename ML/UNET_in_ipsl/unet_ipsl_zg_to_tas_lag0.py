...
class CustomDataset(tf.keras.utils.Sequence):
    def __init__(self, zg_file, tas_file, batch_size=32,lag=0,std_ref_zg=None):
    ...
    def __len__(self):
        return int(np.ceil(len(self.zg_data) / self.batch_size))
    ...
    def __getitem__(self, idx):
        ...
        in_tensor = np.expand_dims(zg, axis=-1)
...
dataset_train = CustomDataset(zg_file_train, tas_file_train, batch_size=64,lag=0)
dataset_val = CustomDataset(zg_file_val, tas_file_val, batch_size=64,lag=0,std_ref_zg=dataset_train.zg_std)
...
callbacks = [
    ...
    keras.callbacks.ModelCheckpoint("models/zg_to_tas_ipsl_lag0.keras", save_best_only=True),
]
...
ds.to_netcdf("models/zg_to_tas_ipsl_lag0.nc")
model.save("models/zg_to_tas_ipsl_lag0.keras")
np.save('models/zg_to_tas_ipsl_lag0.npy',history.history)
